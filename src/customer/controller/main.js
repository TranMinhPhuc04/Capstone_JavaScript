    ...
    phoneList.forEach((phone) => {
        contentHTML += `
            <div class="phone-card">
                <img src="${phone.img}" alt="Phone image" style="width:100%;height:auto;">
                <h4>${phone.name}</h4>
                <p>Price: $${phone.price}</p>
                <button class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded" onclick="addToCart('${phone.id}', event)">Add to Cart</button>
                <button class="bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded" onclick="viewDetails('${phone.id}')">View Details</button>
            </div>
        `;
    });
...
/**
 * Xem chi tiết điện thoại (chỉ hiển thị thông tin, không chỉnh sửa)
 */
        ...
        cartHTML += `
            <div class="cart-item flex justify-between items-center border-b pb-3 mb-3">
                <div class="flex items-center">
                    <img src="${item.phone.img}" alt="Phone image" style="width:40px;height:auto;" />
                    <div class="ml-4">
                        <p>${item.phone.name}</p>
                        <p>Price: $${item.phone.price}</p>
                        <div class="flex items-center">
                          <button class="bg-gray-300 text-black px-2 rounded" onclick="decreaseQuantity('${item.phone.id}')">-</button>
                          <span class="mx-2">${item.quantity}</span>
                          <button class="bg-gray-300 text-black px-2 rounded" onclick="increaseQuantity('${item.phone.id}')">+</button>
                        </div>
                        <p>Total Price: $${item.phone.price * item.quantity}</p>
                    </div>
                </div>
                <button onclick="removeFromCart('${item.phone.id}')" class="text-red-500 hover:text-red-700">
...
const filterProductsByType = (type) => {
    if (type === 'all') {
        ...
        const filteredPhones = listPhone.filter(phone => phone.type.toLowerCase() === type);
